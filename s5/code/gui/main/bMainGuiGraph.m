...
%                 );
%             me.TextAlim = uicontrol(...
%                 'Parent', textsPanel,...
%                 'Style','edit',...
%                 'Units','character',...
%                 'Position',[64,0,10,1.5],...
%                 'String', '',... %num2str(me.Alim),...
%                 'Enable', 'off',...
...
%             if val
%                 set(me.TextAlim,'Enable','on');
%             else
%                 set(me.TextAlim,'Enable','off');
%                 
%             end
%         end
...
%         end
        ...
        function setFileChannel(me,fileIdx,channelIdx)
            ...
            me.setSlider();
%             if ~isempty(me.Alim) && me.MADL
%                 % do nothing
%             else
                me.Alim = fileData(me.FileIdx,'Ylim');
%             end
            me.plot();
        ...
        function plotTS(me,dataset)
            if isempty(dataset)
                ...
                set(me.AxesTS,'Visible','on','Ylim',me.Alim.*1.1);
                me.plotChannelCalls(me.AxesTS,'ccTS','ccTxtTS',true);
                me.plotPoi(me.AxesTS,'poiTS','poiTxtTS');
            end
        end
        ...
        function plotSpec(me,dataset)
            if isempty(dataset)
                ...
                me.plotChannelCalls(me.AxesSpec,'ccSpec','ccTxtSpec',false);
                me.plotPoi(me.AxesSpec,'poiSpec','poiTxtSpec');
            end
        end
        
        % PLOT CHANNEL CALLS
        function plotChannelCalls(me,axobj,plotProp,txtProp,withAmplitude)
            if me.FileIdx == 0
                return;
            end
            
            % clear older marks
            if ishandle(me.(plotProp))
                delete(me.(plotProp));
                me.(plotProp) = NaN;
            end
            if ishandle(me.(txtProp))
                delete(me.(txtProp));
                me.(txtProp) = NaN;
            end
            ...
            Ylim = get(axobj,'Ylim');
            axes(axobj);
            switch ggetParam('mainGui_showChannelCalls')
                case 'dont'
                case 'marks'
                    [D] = channelData(me.FileIdx,me.ChannelIdx,'Calls','Detections','TimeInterval',Tlim);
                    if isempty(D)
                        return;
                    end
                    T = D(:,1);
                    if withAmplitude
                        ...
                        Y = mean(Ylim).*ones(size(T));
                    end
                    hold on
                    me.(plotProp) = plot(T,Y,'r*');
                    set(me.(plotProp),'UIContextMenu',me.Gui.ContextMenu);
                    hold off;
                case 'numbered'
                    [D,I] = channelData(me.FileIdx,me.ChannelIdx,'Calls','Detection','TimeInterval',Tlim);
                    if isempty(D)
                        return;
                    end
                    T = D(:,1);
                    ...
                    TXT = cellfun(@(i) num2str(i),num2cell(I),'UniformOutput',false);
                    hold on
                    me.(plotProp) = plot(T,Y,'r*');
                    set(me.(plotProp),'UIContextMenu',me.Gui.ContextMenu);
                    me.(txtProp) = text(T,Y,TXT);
                    hold off;                        
            end                                            
        end
        
        % PLOT POI
        function plotPoi(me,axobj,plotProp,txtProp)
            if me.FileIdx == 0
                return;
            end
        ...
        function mouseMotionTS(me,z)
            ...
            me.LastPointerPosition = [time,value,0];
        end
        ...
        function mouseMotionSpec(me,z)
            ...
            me.Group.showMouseMarks(time);
            me.LastPointerPosition = [time,0,freq];
        end
